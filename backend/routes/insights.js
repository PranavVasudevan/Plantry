...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    /* =========================
       1. Shopping Rhythm
       ========================= */
    const temporalSnap = await db.collection("model_outputs_temporal").doc(householdId).get();
    ...
    if (temporalSnap.exists) {
      ...
      if (strongest) {
        rhythm = {
          ...
          preferredDay: strongest.pattern ? strongest.pattern.replace("_", " ") : null,
    ...
    /* =========================
       ...
       ========================= */
    const forgetSnap = await db.collection("model_outputs_forgetfulness").doc(householdId).get();
    ...
    /* =========================
       3. Smart Pairs
       ========================= */
    const assocSnap = await db.collection("model_outputs_associations").doc(householdId).get();

    const pairs = [];

    if (assocSnap.exists) {
      Object.entries(assocSnap.data().rules || {}).forEach(([lhs, rules]) => {
        rules.forEach(r => {
          if (r.confidence >= 0.6) {
            pairs.push([capitalize(lhs), capitalize(r.item)]);
          }
        });
      });
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [temporalSnap, forgetSnap] = await Promise.all([
    ...
    const fact = generateRandomFact({ temporal, forgetScores });
